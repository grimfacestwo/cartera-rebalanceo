...
describe("parseContributions", () => {
  it("devuelve array vacío si no es array o es null", () => {
    expect(parseContributions(null)).toEqual([]);
    expect(parseContributions("invalido")).toEqual([]);
    expect(parseContributions({})).toEqual([]);
  });

  it("parsea correctamente contribuciones válidas", () => {
    const raw = [
      { id: "c1", assetId: "msci", amount: "500", date: "2026-09-01", note: "Nómina" },
      { id: "c2", assetId: "oro", amount: 200, date: "2026-09-02" },
      { id: 123, invalid: true }, // se ignora
    ];
    const parsed = parseContributions(raw);
  ...
  it("calcula totalContributed y assetContributed correctamente", () => {
    const list = [
      { id: "1", assetId: "msci", amount: "300", date: "2026-09-01" },
      { id: "2", assetId: "msci", amount: "200", date: "2026-09-15" },
      { id: "3", assetId: "oro", amount: "150", date: "2026-09-10" },
    ];
    expect(totalContributed(list)).toBe(650);
    expect(assetContributed(list, "msci")).toBe(500);
    expect(assetContributed(list, "oro")).toBe(150);
    expect(assetContributed(list, "btc")).toBe(0);
  });
});
