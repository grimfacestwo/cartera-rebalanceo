...
export function parseContributions(raw: unknown): PortfolioContribution[] {
  if (!Array.isArray(raw)) return [];
  const out: PortfolioContribution[] = [];
  for (const item of raw) {
    if (item && typeof item === "object") {
      const o = item as Record<string, unknown>;
      if (
        ...
        const amtStr = typeof o.amount === "number" ? String(o.amount) : o.amount;
        const dateStr =
...
export function totalContributed(contributions: PortfolioContribution[]): number {
  return contributions.reduce((sum, c) => sum + (Number.parseFloat(c.amount) || 0), 0);
}

export function assetContributed(contributions: PortfolioContribution[], assetId: string): number {
