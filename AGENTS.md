...
- **Weekly email backup**: `vercel.json` defines a Vercel Cron (`0 7 * * 1`, Mondays 07:00 UTC) hitting `app/api/cron/backup/route.ts`, which reads `portfolio_state` and emails the same JSON the "Exportar" button produces via the Resend API. Requires three env vars in Vercel (not committed): `CRON_SECRET` (Vercel auto-sends it as `Authorization: Bearer <value>` on cron requests — the route 401s without a match), `RESEND_API_KEY`, and `BACKUP_EMAIL_TO`. `BACKUP_EMAIL_FROM` is optional (defaults to Resend's sandbox sender, which only works if `BACKUP_EMAIL_TO` is the Resend account's own verified address — a real domain needs verifying in Resend for other recipients).
